...
แต่เนื่องจาก `a` ถูกประกาศด้วย `var` จึงทำให้มันเป็น local variable ของฟังก์ชั่น (ในที่นี้คือ [*นิพจน์ฟังก์ชั่น - function expression*](https://developer.mozilla.org/en-US/docs/web/JavaScript/Reference/Operators/function) ที่ถูก
...
JavaScript มีคุณสมบัติ [**Hoisting**](https://www.w3schools.com/js/js_hoisting.asp) หมายถึง การประกาศตัวแปรและฟังก์ชั่นต่างๆจะถูกยกขึ้นมาไว้ข้างบนสุดของ scope นั้นๆ

ในที่นี้ การอ่านค่าตัวแปร `a` และการเรียกใช้ฟังก์ชั่น `foo()` ก่อนการประกาศ 2 สิ่งนี้ จึงสามารถทำได้

แต่ตอนที่ log ค่าของ `a` นั้น `a` ยังไม่ได้ถูกกำหนดค่าเริ่มต้น (เพราะ *การกำหนดค่า* ให้กับ `a` มาทีหลัง ถึงแม้ *การประกาศ* `a` จะมาก่อนเพราะถูก hoist ขึ้นมาก็ตาม) จึงทำให้ `a` มีค่าเป็น `undefined` และแสดงค่านี้ออกมาที่
console
...
ฟังก์ชั่น `Greet()` ในที่นี้คือ [constructor function](https://www.w3schools.com/js/js_object_constructors.asp) และ `Greet` จะเทียบได้กับคลาส (class) ในภาษาแบบ OOP ซึ่ง `new Greet()` จะเป็นการสร้าง `Greet` instance/object
...
ฟังก์ชั่น `ObjSort()` จะรับ array เข้ามาทำการเรียงลำดับข้อมูลจากน้อยไปหามาก โดยใช้ [bubble sort algorithm](https://www.geeksforgeeks.org/bubble-sort/)
...
ที่จริงโค้ด 5 บรรทัดสุดท้ายที่ทำหน้าที่แสดงค่าของ `arrayResult` สามารถยุบให้เหลือบรรทัดเดียว โดยใช้ [array `reduce()` method](https://www.w3schools.com/jsref/jsref_reduce.asp) ดังนี้

```javascript
var result = arrayResult.reduce((acc, item) => acc + item + " ", "");
```

หรือใช้ backquote (\`) ในการทำ [string interpolation](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Template_literals) แทนการต่อสตริงด้วย operator `+` ก็จะได้โค้ดที่สวยและเข้าใจง่ายขึ้นอีก 🎉
